...
export default function ElectionCenterPage() {
  ...
  const activeElection =
    elections.find((e: any) => e.id === activeElectionId) || elections[0] || null;
  ...
  const { data: activeResults } = useQuery({
    ...
    enabled:
      !!activeElection?.id &&
      ['ACTIVE', 'PAST', 'COMPLETED', 'CLOSED'].includes(activeElection.status),
  });
  ...
  const currentCandidates: any[] =
    candidatesList.length > 0 ? candidatesList : activeElection?.positions?.[0]?.candidates || [];
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Manage student government and guild elections securely.
          </p>
        </div>
          ...
          {activeElection ? (
            <Card className="premium-card border-none overflow-hidden rounded-3xl shadow-xl bg-card text-card-foreground">
              ...
              <div className="h-48 md:h-64 w-full relative">
                <img
                  src={
                    activeElection.coverImage ||
                    'https://images.unsplash.com/photo-1540910419892-f3174207baec?q=80&w=2070&auto=format&fit=crop'
                  }
                ...
                <div className="absolute bottom-6 left-6 right-6">
                  <div className="flex items-center gap-3 mb-2">
                    <Badge
                      className={`rounded-full px-3 py-1 font-bold tracking-wider text-[10px] ${
                    ...
                    <Badge
                      variant="outline"
                      className="bg-white/10 backdrop-blur-md border-white/20 text-white rounded-full px-3 py-1 font-bold tracking-wider text-[10px] uppercase"
                    >
                      {activeElection.category?.replace('_', ' ')}
                    </Badge>
                  </div>
                  <h3 className="text-3xl font-black text-white tracking-tight font-heading leading-tight">
                ...
                <div className="absolute top-6 right-6 flex gap-2">
                  {(activeElection.status === 'UPCOMING' || activeElection.status === 'DRAFT') && (
                    <Button
                      onClick={() =>
                        updateStatusMutation.mutate({ id: activeElection.id, status: 'ACTIVE' })
                      }
                  ...
                  {activeElection.status === 'ACTIVE' && (
                    <Button
                      onClick={() =>
                        updateStatusMutation.mutate({ id: activeElection.id, status: 'CLOSED' })
                      }
                  ...
                  {activeElection.status === 'CLOSED' && (
                    <Button
                      onClick={() =>
                        updateStatusMutation.mutate({ id: activeElection.id, status: 'COMPLETED' })
                      }
                ...
                <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                  <div className="border-b border-border bg-muted/30 px-6">
                    <TabsList className="bg-transparent border-0 h-14 p-0 gap-6">
                      <TabsTrigger
                        value="overview"
                        className="data-[state=active]:bg-transparent data-[state=active]:text-primary data-[state=active]:shadow-none data-[state=active]:border-b-2 data-[state=active]:border-primary rounded-none h-14 font-bold text-muted-foreground"
                      >
                        Overview
                      </TabsTrigger>
                      <TabsTrigger
                        ...
                        <div className="flex-1 space-y-4">
                          ...
                          <p className="text-muted-foreground leading-relaxed whitespace-pre-line">
                            {activeElection.description ||
                              'No detailed description provided for this election.'}
                          </p>
                          <div className="grid grid-cols-2 gap-4 mt-6">
                            ...
                            <div className="bg-muted/50 p-4 rounded-2xl border border-border">
                              ...
                              <p className="text-sm font-bold text-foreground">
                                {activeElection.isAnonymous !== false
                                  ? 'Encrypted & Anonymous'
                                  : 'Public Record'}
                              </p>
                            </div>
                        ...
                        <div className="w-full md:w-72 space-y-4">
                          <Card className="bg-muted/50 border-none rounded-2xl p-4 shadow-sm">
                            <h5 className="font-bold text-foreground mb-4 flex items-center">
                              <span className="material-symbols-rounded text-primary mr-2 text-[20px]">
                                schedule
                              </span>{' '}
                              Timeline
                            </h5>
                          ...
                          <div className="bg-primary/5 p-4 rounded-2xl border border-primary/10">
                            <p className="text-xs font-medium text-primary mb-2 flex items-center">
                              <span className="material-symbols-rounded text-[16px] mr-1.5">
                                info
                              </span>{' '}
                              Eligibility Info
                            </p>
                            <p className="text-xs text-muted-foreground font-medium">
                              This election is restricted to {activeElection.category} members.
                              Verification is{' '}
                              {activeElection.requiresVerification !== false
                                ? 'required'
                                : 'optional'}
                              .
                            </p>
                    ...
                    <TabsContent value="candidates" className="mt-0 space-y-4">
                      ...
                      <div className="grid gap-4 sm:grid-cols-2">
                        {currentCandidates.map((candidate: any) => (
                          <div
                            key={candidate.id}
                            className="group flex flex-col p-4 bg-card border border-border rounded-3xl hover:shadow-lg hover:border-primary/20 transition-all"
                          >
                            <div className="flex items-center gap-4 mb-3">
                              <div className="h-14 w-14 rounded-2xl overflow-hidden bg-muted border border-border shrink-0">
                                {candidate.photo || candidate.imageUrl ? (
                                  <img
                                    src={candidate.photo || candidate.imageUrl}
                                    alt={candidate.name}
                                    className="w-full h-full object-cover"
                                  />
                                ) : (
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <Badge
                                  variant="secondary"
                                  className="mt-1 bg-muted text-muted-foreground text-[10px] font-bold uppercase rounded-full"
                                >
                        ...
                        {currentCandidates.length === 0 && (
                          <div className="col-span-full py-20 text-center">
                            <span className="material-symbols-rounded text-5xl text-muted/30 mb-4 block">
                              person_search
                            </span>
                    ...
                    <TabsContent value="results" className="mt-0">
                      <div className="space-y-6">
                        <div className="bg-muted/30 rounded-3xl p-6 border border-border">
                          <div className="flex justify-between items-end mb-6">
                            <div>
                              <div className="text-sm font-bold text-muted-foreground mb-2 flex items-center uppercase tracking-widest">
                                <span className="material-symbols-rounded text-[18px] mr-2 text-primary">
                                  analytics
                                </span>
                                Voter Participation
                              </div>
                          ...
                          <Progress
                            value={resultsData?.turnout || 0}
                            className="h-4 bg-muted rounded-full [&>div]:bg-gradient-to-r [&>div]:from-primary [&>div]:to-emerald-500"
                          />
                        </div>
            ...
            <Card className="premium-card border-dashed border-2 flex items-center justify-center p-20 bg-muted/20 rounded-3xl border-border">
              <div className="text-center">
                <span className="material-symbols-rounded text-6xl text-muted-foreground/30 mb-6 block">
                  how_to_vote
                </span>
                <p className="text-muted-foreground font-bold text-lg">
                  No elections found in your jurisdiction
                </p>
                <CreateElectionDialog
                  onElectionCreated={() =>
                    queryClient.invalidateQueries({ queryKey: ['admin', 'elections'] })
          ...
          <Card className="premium-card border-none flex flex-col rounded-3xl overflow-hidden shadow-lg h-full max-h-[800px] bg-card text-card-foreground">
            <CardHeader className="border-b border-border pb-4 bg-muted/30">
              <CardTitle className="text-lg font-black text-foreground flex items-center justify-between tracking-tight">
                ...
                <Badge
                  variant="secondary"
                  className="rounded-full font-black bg-background border border-border shadow-sm"
                >
                  {elections.length}
                </Badge>
              </CardTitle>
              ...
              <div className="divide-y divide-border h-full">
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between mb-3">
                        <span
                          className={`font-black tracking-tight leading-tight transition-colors ${activeElectionId === election.id ? 'text-primary' : 'text-foreground group-hover:text-primary'}`}
                        >
                          {election.title}
                        </span>
                        <Badge
                      ...
                      <div className="flex items-center justify-between gap-2">
                        <Badge
                          variant="outline"
                          className="text-[9px] uppercase font-bold border-border bg-background rounded-full"
                        >
                ...
                {elections.length === 0 && (
                  <div className="p-20 text-center text-muted/30">
                    <span className="material-symbols-rounded text-4xl mb-2 opacity-20 block">
                      inventory_2
                    </span>
